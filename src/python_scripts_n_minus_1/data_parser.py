...
class DataParser:
    def __init__(self, parameters):
        ...
        self.raw_data["adj_mats"] = np.stack([self.raw_data["adj_mat"]] * len(self.raw_data["node_attributes"]))
        self.raw_data["imp_mats"] = np.stack([self.raw_data["imp_mat"]] * len(self.raw_data["node_attributes"]))
        del(self.raw_data["adj_mat"])
        del(self.raw_data["imp_mat"])
        raw_data_test = load_json(self.samples_test)
        for key in self.raw_data.keys():
            self.raw_data[key] = [*self.raw_data[key], *raw_data_test[key]]
        for key in self.raw_data:
            setattr(self, key, self.raw_data[key])
        self.num_samples = len(self.node_attributes)
        self.num_features = len(self.features)
        self.num_bus = len(self.adj_mats[0][0])
        ...
        self.imp_mats = self._update_imp_mats()
        self.imp_index = []
        self.imp_values = []
        for k in range(self.num_samples):
            imp_index, imp_values = to_sparse_matrix(self.imp_mats[k])
            self.imp_index.append(imp_index)
            self.imp_values.append(np.exp(-self.kernel_wt * np.log(imp_values)))
            ...
            for i in range(n):
                for j in range(i+1, n):
                    if self.gen_bus_indicies[i] == self.gen_bus_indicies[j]:
                        ...
                        imp_mat[i, j] = self.imp_mats[k][self.gen_bus_indicies[i] - 1][self.gen_bus_indicies[j] - 1]
    ...
    def _parse_gnn_input(self):
        ...
        for i in range(self.num_samples):
            edge_index.append(add_self_loops(self.imp_index[i], self.imp_values[i])[0])
            if self.architecture == "snn":
                edge_attr.append(
                    add_self_loops(self.imp_index[i], self.imp_values[i])[1].unsqueeze(1)
                )
            else:
                edge_attr.append(add_self_loops(self.imp_index[i], self.imp_values[i])[1])
    ...
    def _set_indicies(self):
        ...
        num_samples = self.num_samples - 1000
        indicies = np.random.permutation(num_samples)
        self.indicies_train = indicies[0 : round(num_samples * self.train_size)]
        self.indicies_val = indicies[
            round(num_samples * self.train_size) : round(
                num_samples * (1 - self.test_size)
            )
        ]
#       self.indicies_test = indicies[
#           round(self.num_samples * (1 - self.test_size)) : self.num_samples
#       ]
        self.indicies_test = np.array(range(10000, 11000))
